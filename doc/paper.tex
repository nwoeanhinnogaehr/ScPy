...
def fn(x):
    ...
    reorganizes the spectrum to move the new frequencies into the appropraite bins.}
    \label{lst:shepard}
\end{listing}
...
SuperCollider's FFT operations are referred to as phase vocoder operation, there is actually no
implementation of a phase vocoder to be found in SuperCollider at the time of writing. It is likely
that a phase vocoder was once intended to be implemented, but the 32 so-called phase vocoder
effects are simply transformations to the cartesian or polar representations of the spectrum.
...
Support for more SuperCollider types, such as events or strings could additionally be added.
