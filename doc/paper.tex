...
\cite{systemsinterfacingwithsc}~\cite{magnusson2011ixi}~\cite{orlarey2009faust}, they are mostly
...
However, it is designed primarily for performing operations on control rate data arrays and its
author notes performance issues which prevent it from being used on audio rate data.
...
and numerical computing. With a few small exceptions, these libraries contained every one of the
fundamental spectral operations we could think of.
